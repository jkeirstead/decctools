...
#'	\item short_code: the UK geography local administrative district short code (LAD11CDO)
#' 	\item long_code: the UK geography local administrative district long code (LAD11CD)
#'	\item urban_class: the urban/rural classification
#'	\item group_name: a descriptive label describing the character of each LAD
...
#' }
#'
#' @details There are multiple subnational geographies available for the UK; please see \url{http://www.ons.gov.uk/ons/guide-method/geography/beginner-s-guide/index.html} for an overview.  This data set tries to make it easier to handle the links between two of these geographies: local administrative districts (LADs, the UK administrative geography for local authorities) and local administrative units (LAUs, the pan-European equivalent for EuroStat).
#'
#' The first five data fields, and the \code{area} field, should be self-explanatory and can be used to link other data sets, most notably the energy statistics provided by \code{\link{get_LAD_data}}.  However the other items require a bit of explanation.
#' 
#' \code{urban_class}: These classifications are described at \url{http://www.ons.gov.uk/ons/guide-method/geography/products/area-classifications/rural-urban-definition-and-la/rural-urban-local-authority--la--classification--england-/index.html} and are summarized as follows:
#' \itemize{
...
#'
#' \code{hdd}, \code{cdd}: These fields represent annual heating and cooling degree days for each LAD; the raw data come from \url{http://www.eci.ox.ac.uk/research/energy/degreedays.php}.  Heating degree days are assumed to be measured against a 15.5$^\circ$C base temperature and cooling degree days against 13$^\circ$C.  The Oxford data set provides point estimates for 77 locations around the UK; an inverse distance weighting interpolation performed (with $p = 0.2$) to give a smooth surface of degree days across the UK and this was sampled using the LAD polygons to estimate the heating and cooling degree days in each location.
#'
